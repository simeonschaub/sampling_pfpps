...
# ╔═╡ 7438b30d-19ec-4a00-afeb-41655243622a
begin
...
# ╔═╡ c04d910b-4fe4-46ae-bb3e-b19fb033e6e7
using CairoMakie
...
# ╔═╡ ef45e886-0238-4153-9b24-c249cb8fd3a3
begin
	eval(:(import Pkg; Pkg.add(; url = "https://github.com/simeonschaub/FredholmDeterminants.jl")))
	using FredholmDeterminants: TracyWidom
end

# ╔═╡ e35e1679-971e-470e-9bf5-1efd6283485b
N = 10

# ╔═╡ bb812b69-be84-484c-a5be-599795c0ac48
ε(x, y) = sign(y - x) / 2

# ╔═╡ ec4fec4a-8c22-450a-9721-c4b21f5957f6
w(x) = exp(-x^2 / 4)

# ╔═╡ 6fe0a808-509e-4258-8d6d-27a1c360b084
# ╠═╡ disabled = true
#=╠═╡
function skew_dot(f, g)
	integrand((x, y), p) = ε(x, y) * w(x) * w(y) * f(x) * g(y)
	prob = IntegralProblem(integrand, ([-Inf, -Inf], [Inf, Inf]))
	sol = solve(prob, HCubatureJL(); reltol = 1e-9, abstol = 1e-9)
	return sol.u
end
  ╠═╡ =#
...
function skew_dot(f, g)
	inner(y, p) = w(y) * g(y)
	inner_prob = init(IntegralProblem(inner, (-Inf, Inf)), QuadGKJL(; buffer = true))

	function integrand(x, upper)
		inner_prob.domain = upper ? (x, Inf) : (-Inf, x)
		return w(x) * f(x) * solve!(inner_prob).u
	end
	prob = init(IntegralProblem(integrand, (-Inf, Inf), true), QuadGKJL(; buffer = true))
	upper = solve!(prob).u
	prob.p = false
	lower = solve!(prob).u
...
let x = Fun(Hermite())
	global A(f) = SymplecticFormVec(x * f[], f.skewf)
end

# ╔═╡ 31f01e7f-48a0-4105-8379-217ead9d7031
begin
	v₀ = SymplecticFormVec(Fun(Hermite(), [1.0]), skew_dot)
	itr = ArnoldiIterator(A, v₀, ModifiedSymplecticGramSchmidt2(ESR3m))
	fact = initialize(itr)
	for i in 1:(N - 1)
		expand!(itr, fact)
	end
	H = [rayleighquotient(fact); zeros(1, N - 1) normres(fact)]
	W = getindex.(basis(fact).basis)
end

# ╔═╡ a4b08932-9ea9-4d25-a052-4060e7bd5bee
begin
	binedges = -8:.2:8
	domain = (first(binedges) + step(binedges) / 2):step(binedges):(last(binedges) - step(binedges) / 2)
end
...
# ╔═╡ 3fefbdc2-4186-4df6-b304-4444f26b3e0a
const w⁻¹∂ₓw = (Derivative() - Fun(Hermite(), [0, 0.25]))

# ╔═╡ 84af36ec-20e5-4544-a4ae-588bd0271677
WGLMakie.activate!()

# ╔═╡ 9ea0dc81-8b39-4d49-9892-66fbd3be0292
skew_dot.(W, permutedims(W))
...
let N = 6
	...
	ax2 = Axis(fig[2, 1]; limits = ((-5, 5), (-.1, 1.2)), xlabel = L"x")
	...
	Label(fig[0, 1], L"$\beta = 1$\textbf{ Skew-Orthogonal Polynomials w.r.t. }$w(x) = e^{-x^2 / 4}$"; tellwidth = false, font = ax.titlefont, fontsize = 18)

	save("goe_sop.pdf", fig)
...
let W = W, N = N
	...
	ϕ(x, k) = w(x) * W[k + 1](x)
	function ψ(x, k; prob = init(IntegralProblem(ϕ, (-Inf, Inf), k), QuadGKJL(; buffer = true)))
		prob.p = k
		prob.domain = (-Inf, x)
		I1 = solve!(prob).u
		prob.domain = (x, Inf)
    ...
    global function S₁(x, y; prob = init(IntegralProblem(ϕ, (-Inf, Inf), 1), QuadGKJL(; buffer = true)))
		return w(y) * sum(0:(N ÷ 2 - 1)) do k
        	ψ(x, 2k; prob) * W[2k + 2](y) - ψ(x, 2k + 1; prob) * W[2k + 1](y)
		end
    end
    D₁(x, y) = w(x) * w(y) * sum(0:(N ÷ 2 - 1)) do k
        W[2k + 1](x) * W[2k + 2](y) - W[2k + 2](x) * W[2k + 1](y)
    end
	S₁ᵀ(z, (x, S_prob)) = S₁(x, z; prob = S_prob)
	global function I₁(x, y;
				S_prob = init(IntegralProblem(ϕ, (-Inf, Inf), 1), QuadGKJL(; buffer = true)),
				prob = init(IntegralProblem(S₁ᵀ, (-Inf, Inf), (x, S_prob)), QuadGKJL(; buffer = true)),
			   )
		prob.p = (x, S_prob)
		prob.domain = (-Inf, y)
		I1 = solve!(prob).u
		prob.domain = (y, Inf)
		I2 = solve!(prob).u
		return (I2 - I1) / 2 - ε(y, x)
    end

    @tasks for j in eachindex(domain)
		Δ = step(domain)
		y = domain[j]
		@local (S_prob, prob) = let
			S_prob = init(IntegralProblem(ϕ, (-Inf, Inf), 1), QuadGKJL(; buffer = true))
			S_prob, init(IntegralProblem(S₁ᵀ, (-Inf, Inf), (0.0, S_prob)), QuadGKJL(; buffer = true))
		end
		for (i, x) in enumerate(domain)
	        f[2i - 1, 2j - 1] = S₁(x, y; prob = S_prob) * Δ
	        f[2i, 2j - 1] = D₁(x, y) * Δ
	        f[2i - 1, 2j] = I₁(x, y; prob, S_prob) * Δ
	        f[2i, 2j] = S₁(y, x; prob = S_prob) * Δ
		end
    end
...
# ╔═╡ ba30b435-7bc9-4292-b508-178dab7052dd
# ╠═╡ disabled = true
#=╠═╡
let N = N
	W = [Fun(Hermite(), [zeros(j); 1 / √(exp2(j) * factorial(j) * √π)]) for j in 0:(N - 1)]
	global f = Matrix{Float64}(undef, 2length(domain), 2length(domain))
	
	φ(x, k) = w(x) * W[k + 1](x)
	function Φ(x, k; prob = init(IntegralProblem(φ, (-Inf, Inf), k), QuadGKJL(; buffer = true)))
		prob.p = k
		prob.domain = (-Inf, x)
		I1 = solve!(prob).u
		prob.domain = (x, Inf)
	...
	w⁻¹∂ₓw = (Derivative() - Fun(Hermite(), [0, 0.25]))

    global function S₁(x, y; prob = init(IntegralProblem(φ, (-Inf, Inf), 0), QuadGKJL(; buffer = true)))
		return w(x) * sum(0:(N ÷ 2 - 1)) do i
        	W[2i + 1](x) * φ(y, 2i) - (w⁻¹∂ₓw * W[2i + 1])(x) * Φ(y, 2i; prob)
		end
    end
    D₁(x, y) = w(x) * w(y) * sum(0:(N ÷ 2 - 1)) do i
        (w⁻¹∂ₓw * W[2i + 1])(x) * W[2i + 1](y) - W[2i + 1](x) * (w⁻¹∂ₓw * W[2i + 1])(y)
    end
    global function I₁(x, y; prob = init(IntegralProblem(φ, (-Inf, Inf), 0), QuadGKJL(; buffer = true)))
		return sum(0:(N ÷ 2 - 1)) do i
        	Φ(x, 2i; prob) * φ(y, 2i) - φ(x, 2i) * Φ(y, 2i; prob)
		end
    end
    ...
    @tasks for j in eachindex(domain)
		Δ = step(domain)
		y = domain[j]
		@local prob = init(IntegralProblem(φ, (-Inf, Inf), 0), QuadGKJL(; buffer = true))
		for (i, x) in enumerate(domain)
	        f[2i - 1, 2j - 1] = S₁(x, y; prob) * Δ
	        f[2i, 2j - 1] = I₁(x, y; prob) * Δ
	        f[2i - 1, 2j] = D₁(x, y) * Δ
	        f[2i, 2j] = S₁(y, x; prob) * Δ
		end
    end
	global K = JMatrix(2length(domain)) * f
end
...
# ╔═╡ b90c61fb-dfdb-4db8-a32f-c06417679db2
let W = W, N = N
	# see https://www.brandeis.edu/mathematics/people/adlerpublications/22.skeworthogpol+randommat.pdf Eq (2.8)
	global f = Matrix{Float64}(undef, 2length(domain), 2length(domain))
	
	global ϕ(x, k) = w(x) * W[k + 1](x)
	global function ψ(x, k; prob = init(IntegralProblem(ϕ, (-Inf, Inf), k), QuadGKJL(; buffer = true)))
		prob.p = k
		prob.domain = (-Inf, x)
		I1 = solve!(prob).u
		prob.domain = (x, Inf)
    ...
    global function S₁(x, y; prob = init(IntegralProblem(ϕ, (-Inf, Inf), 0), QuadGKJL(; buffer = true)))
		return w(x) * sum(0:(N ÷ 2 - 1)) do k
        	W[2k + 2](x) * ψ(y, 2k; prob) - W[2k + 1](x) * ψ(y, 2k + 1; prob)
		end
    end
    global D₁(x, y) = w(x) * w(y) * sum(0:(N ÷ 2 - 1)) do k
        W[2k + 1](x) * W[2k + 2](y) - W[2k + 2](x) * W[2k + 1](y)
    end
    global function J₁(x, y; prob = init(IntegralProblem(ϕ, (-Inf, Inf), 0), QuadGKJL(; buffer = true)))
		return sum(0:(N ÷ 2 - 1)) do k
        	ψ(x, 2k + 1; prob) * ψ(y, 2k; prob) - ψ(x, 2k; prob) * ψ(y, 2k + 1; prob)
		end - ε(y, x)
    end
    ...
    @tasks for j in eachindex(domain)
		Δ = step(domain)
		y = domain[j]
		@local prob = init(IntegralProblem(ϕ, (-Inf, Inf), 0), QuadGKJL(; buffer = true))
		for (i, x) in enumerate(domain)
	        f[2i - 1, 2j - 1] = S₁(x, y; prob) * Δ
	        f[2i, 2j - 1] = J₁(x, y; prob) * Δ
	        f[2i - 1, 2j] = D₁(x, y) * Δ
	        f[2i, 2j] = S₁(y, x; prob) * Δ
		end
    end
...
# ╔═╡ dcf2c5ad-2936-4fdb-8943-a572daff625e
function dS₁(x; prob = init(IntegralProblem(ϕ, (-Inf, Inf), 0), QuadGKJL(; buffer = true)))
	return w(x) * sum(0:(N ÷ 2 - 1)) do k
		 (w⁻¹∂ₓw * W[2k + 2])(x) * ψ(x, 2k; prob) - (w⁻¹∂ₓw * W[2k + 1])(x) * ψ(x, 2k + 1; prob)
	end
end

# ╔═╡ 94148d8b-1a0a-403e-b240-38fa0b0aac76
let
	prob = init(IntegralProblem(ϕ, (-Inf, Inf), 0), QuadGKJL(; buffer = true); abstol = 1e-5, reltol = 1e-5)
	obj = ARS.Objective(x -> log(S₁(x, x; prob)), x -> dS₁(x; prob) / S₁(x, x; prob))
	sam = ARS.ARSampler(obj, [-5.0, 5.0], (-Inf, Inf))
	fig = hist(ARS.sample!(sam, 1000000); normalization = :pdf, bins = 100)
	lines!(-7..7, s -> S₁(s, s; prob) / N)
	fig
end

# ╔═╡ d0b3f97a-f1eb-4adf-974f-e04bb033b8dd
begin
	struct RPCholCache{T, P, S}
		n::Int
		prob::P
		sam::S
		S::Vector{T}
	...
	function RPCholCache(n::Int)
		...
		obj = ARS.Objective(x -> log(S₁(x, x; prob)), x -> dS₁(x; prob) / S₁(x, x; prob))
		sam = ARS.ARSampler(obj, [-5.0, 5.0], (-Inf, Inf))
		return RPCholCache(n, prob, sam, Vector{Float64}(undef, n),
...
# ╔═╡ 844c88b3-eac7-4e3f-afcd-7a7561b2af50
let
	fig = Figure(; size = (1200, 500))
	for i in 1:3
		surface(fig[1, i], domain, domain, K[fld1(i, 2):2:end, mod1(i, 2):2:end];
			axis = (; type = Axis3, azimuth = 0.1π, elevation = 0.25π),
		)
		i == 2 && lines!(Point3f.(domain, domain, S₁.(domain, domain) .* step(domain)); color = :red, linewidth = 2)
		Label(fig[0, i],
			[rich("K", subscript("11"), "(x, y)"), rich("K", subscript("12"), "(x, y) = -K", subscript("21"), "(y, x)"), rich("K", subscript("22"), "(x, y)")][i];
			font = current_axis().titlefont, fontsize = 28, tellwidth = false
		)
	end
	save("goe_pfpp_kernel.png", fig; px_per_unit = 2)
	fig
end

# ╔═╡ cc484c00-8c15-4882-b929-fa25501497f1
let
	fig = surface(domain, domain, K[1:2:end, 2:2:end]; axis = (; type = Axis3))
	lines!(Point3f.(domain, domain, S₁.(domain, domain) .* step(domain)); color = :red, linewidth = 3)
	fig
end

# ╔═╡ d652be5a-ba63-4d3c-85af-f2bc2c7afbf7
surface(domain, domain, K[1:2:end, 1:2:end]; axis = (; type = Axis3))

# ╔═╡ 70009a1d-8427-447a-bba0-762de3e1cb22
J₁(-100., 100.)

# ╔═╡ b699496a-b200-4195-9e1e-2aee0a07c0df
surface(domain, domain, -K[2:2:end, 2:2:end]; axis = (; type = Axis3))

# ╔═╡ 4fabb891-d412-4685-bd1b-1f1be9c0cefe
...
# ╔═╡ 8242517b-468f-48d0-bfb7-9b48d8510353
let
	fig = Figure(; size = (500, 500))
	surface(fig[1, 1], domain, domain, [K[2j - 1, 2j] - dot(K[(2i - 1):2i, 2j - 1], JMatrix(2), K[(2i - 1):2i, 2j]) / K[2i - 1, 2i] for i in eachindex(domain), j in eachindex(domain)];
		axis = (; type = Axis3, azimuth = 0.25π, elevation = 0.15π),
    )
	Label(fig[0, 1], "P(y | x)";
		font = current_axis().titlefont, fontsize = 28, tellwidth = false
	)
	save("goe_conditional.png", fig; px_per_unit = 2)
	fig
end

# ╔═╡ 738a222e-1c9d-4caf-b331-b8e0108e30e1
sum([K[2j - 1, 2j] - dot(K[(2i - 1):2i, 2j - 1], JMatrix(2), K[(2i - 1):2i, 2j]) / K[2i - 1, 2i] for i in eachindex(domain), j in eachindex(domain)]; dims = 2)

# ╔═╡ a5df0915-53be-4491-8579-94ea83c11ad9
begin
	function fill_kernel!(K, X, Y, m = length(X), n = length(Y); prob = init(IntegralProblem(ϕ, (-Inf, Inf), 0), QuadGKJL(; buffer = true); abstol = 1e-5, reltol = 1e-5))
		for i in 1:m, j in 1:n
			x, y = X[i], Y[j]
			K[2i - 1, 2j - 1] = J₁(x, y; prob)
			K[2i, 2j - 1] = -S₁(x, y; prob)
			K[2i - 1, 2j] = S₁(y, x; prob)
			K[2i, 2j] = -D₁(x, y)
	    end
		return K
	end
	function evaluate_conditioned_pfpp!(k, L, S, s, i = length(S); prob = init(IntegralProblem(ϕ, (-Inf, Inf), 0), QuadGKJL(; buffer = true); abstol = 1e-5, reltol = 1e-5))
		fill_kernel!(k, S, s, i, 1; prob)
		_k = view(k, 1:2i, :)
...
# ╔═╡ 7f88711f-15d0-4770-9a9c-e0e12df02084
function randPfPPRPChol!((; n, prob, sam, S, L, k)::RPCholCache{T}; max_iters = 1000) where {T}
    i = 0
	L .= zero(T)

    for _ in 1:max_iters
		s = ARS.sample!(sam, 1)[]
		d, kₛₛ = evaluate_conditioned_pfpp!(k, L, S, s, i; prob)
		p = d / 2kₛₛ
...
# ╔═╡ 44d51f27-8bcc-4d8e-994b-34cdddd64da0
randPfPPRPChol!(RPCholCache(N))
...
# ╔═╡ e62dd7c5-78fb-4fee-a8c6-e4005b373efb
begin
	binedges′ = (binedges .- 2√N) .* N^(1 / 6)
	domain′ = (domain .- 2√N) .* N^(1 / 6)
end

# ╔═╡ a0e3e0f1-817b-4d7f-8780-bf05eeed0969
begin
	...
	@tasks for _ in 1:10000
		...
		for i in 1:50
			...
			if length(h) == N
				atomic_push!.(@view(hists1[:, i]), (domain[reverse(h)] .- 2√N) .* N^(1 / 6))
			end
		end
	end
...
# ╔═╡ 4f3f1cb0-da69-4bee-b87c-f3dc36dbcce0
begin
	hists2 = [Hist1D(; counttype = Int, binedges = binedges′) for _ in 1:N, _ in 1:50]
	@tasks for _ in 1:10000
		for i in 1:50
			λ = (2√N * eigvals(rand(GaussianHermite(1), N)) .- 2√N) .* N^(1 / 6)
...
let
	...
	ax = Axis(fig[1, 1]; yscale = _log10, limits = (extrema(domain′), (1e-5, 1)))
	tightlimits!(ax)
	for i in 1:N
...
# ╔═╡ 1d132424-d3f8-48b8-9bff-08bf896ce3d8
# ╠═╡ disabled = true
#=╠═╡
data = let c = RPCholCache(N)
	[randPfPPRPChol!(c)[end] for _ in 1:50000]
end
  ╠═╡ =#

# ╔═╡ 11b4d375-0b5e-475d-b2b5-41322752a6e2
#=╠═╡
data = vec(maximum(gibbs_data; dims = 1))
  ╠═╡ =#

# ╔═╡ 63775355-88f3-4358-8740-933d7eadc9ee
CairoMakie.activate!()

# ╔═╡ 439390ab-4e4b-4a01-9806-5c06b331dbc8
let
	fig = Figure(; size = (500, 350))
	ax = Axis(fig[1, 1]; limits = ((-6, 3), (0, 1.1 * maximum(bincounts(normalize(hists2_mean[1]))))), xticks = -6:3) # (2, 9)

	#hist!(ax, data; bins = binedges, normalization = :pdf)
	...
	x = 0:(length(domain) - 1)
	y = map(x) do k
		pfaffian(skewhermitian!(JMatrix(2(length(domain) - k)) - K[(2k + 1):2length(domain), (2k + 1):2length(domain)])) / step(domain)
	end
	stairs!(ax, binedges′, diff([0.0; y; 1.0]) ./ N^(1 / 6); color = :yellow, linewidth = 2, linestyle = :dot)

	x = range(-6, 3; length = 257)
	lines!(ax, x, pdf.(TracyWidom{1}(), x); color = Cycled(3), linewidth = 2)
	...
	], ["PfPP", "GOE eigmax", "Fredholm Pf", "Tracy Widom"])
	Label(fig[0, 1], "Maximum GOE eigenvalues (N = $N) sampled\nfrom PfPP and directly"; tellwidth = false, font = ax.titlefont, fontsize = 18)
	
	save("goe_hist.pdf", fig)
	
	fig
end

# ╔═╡ 8d2ae48f-6733-47e4-9674-bac09bc5a8e1
function randPfPPGibbs(S₀, T; w = 1.0)
	...
	for j in 1:(N - 1)
		s = S₀[j]
		d, _ = evaluate_conditioned_pfpp!(k, L, S₀, s, j - 1; prob)
	...
	for t in 2:(T + 1)
		...
		for i in N:-1:1
			for j in i:(N - 1)
				s = S[j + 1, t]
				d, _ = evaluate_conditioned_pfpp!(k, L, view(S, :, t), s, j - 1; prob)
				update_cholesky!(L, k, √d, j)
				S[j, t] = s
			end

    		g(s) = log(max(evaluate_conditioned_pfpp!(k, L, view(S, :, t), s, N - 1; prob)[1], 0.0))
...
# ╔═╡ c4a92e68-c5d0-4f6b-a44b-f4c7c31df038
# ╠═╡ disabled = true
#=╠═╡
gibbs_data = randPfPPGibbs(range(-3, 3; length = N), 10000)
  ╠═╡ =#

# ╔═╡ 88720551-b74c-4d01-b186-65465657e75a
Y′ = let (λ, Q) = eigen(f; sortby = abs)
	Q[:, isapprox.(λ, 1; rtol = .2)]
end

# ╔═╡ adf87614-035f-48d9-aca9-c1d008e9c232
let (λ, Q) = eigen(f; sortby = abs)
	global Z′ = inv(Q)[isapprox.(λ, 1; rtol = .2), :]
	Q * Diagonal(λ) / Q, f, Y′ * Z′
end

# ╔═╡ 749eddd3-a58e-434b-a109-a4b5725d1f6a
...
# ╔═╡ 3e9cf9ee-6a1f-4a17-9766-5c91f8506c20
let (_Q, R) = sr(Y′)
	Q = _Q[:, 1:2N]
...
# ╔═╡ 36fc6fd8-220f-4476-92a4-73f6fa639d37
Z′

# ╔═╡ c9d9204f-df68-49a4-9f65-d34b9081c8a7
function randPfPPproj(Y, Z)
    ...
    @views for k in 1:n
		...
		p = real.(sum.(Iterators.map.(*, eachrow(Y)[1:2:end], eachcol(Z)[1:2:end])))
        
        # Normalize and sample (max is used to scrub tiny negative floating point errors)
        p = max.(p, 0.0)
        𝓘[k] = rand(Distributions.Categorical(p ./ sum(p)))#(n - k + 1)))
        i = 𝓘[k]
...
# ╔═╡ ae34d92d-fc51-4e9a-b376-cf07aa4702a9
let (_Q, R) = sr(Y′)
	Q = _Q[:, 1:2N] |> real
	Q * -JMatrix(20) * Q' * JMatrix(160), f
	randPfPPproj(Q, -JMatrix(20) * Q' * JMatrix(160))
end

# ╔═╡ 8a7491c2-c8af-4b6b-b79e-e4bb82b4a83f
randPfPPproj(Y′, Z′)

# ╔═╡ 699edaf6-9104-4d44-b6d1-34f78dac8114
real.(Z′) * real.(Y′)

# ╔═╡ 63973ffa-6591-41c9-9c99-cac8762546a4
let
	binedges = -10:.05:10
	domain = (first(binedges) + step(binedges) / 2):step(binedges):(last(binedges) - step(binedges) / 2)
...
# ╔═╡ 8def068f-6ee9-4a7c-b0f2-e845fcb28621
begin
	hists5 = [Hist1D(; counttype = Int, binedges = binedges′) for _ in 1:N, _ in 1:50]
	let
		Y, Z = let (_Q, R) = sr(let (λ, Q) = eigen(f′; sortby = abs)
				Q[:, isapprox.(λ, 1; rtol = .2)]
			end) #Y′)
			Q = _Q[:, 1:2N] |> real
			Q, -JMatrix(20) * Q' * JMatrix(800)
		end#Y′, Z′
		binedges = -10:.05:10
		domain = (first(binedges) + step(binedges) / 2):step(binedges):(last(binedges) - step(binedges) / 2)
		@tasks for _ in 1:10000
			for i in 1:50
				h = randPfPPproj(Y, Z)
				atomic_push!.(@view(hists5[:, i]), (domain[reverse(h)] .- 2√N) .* N^(1 / 6))
			end
		end
	end
...
# ╔═╡ a73a43ac-1db9-4b09-a68f-ac6c4737da6c
let (hists1_mean, hists1_errors) = (hists5_mean, hists5_errors)
	...
	Label(fig[0, 1], "Maximum GOE eigenvalues (N = $N) sampled from PfPP and directly"; tellwidth = false, font = ax.titlefont)
	
	fig
end

# ╔═╡ 5bd794a8-1e1c-4ff8-8e9d-c275d5b91dbc
eigvals(f′; sortby = abs)[(end - 2N):end]

# ╔═╡ 87d0c8ce-e03d-480e-963b-751ec9395b9c
scatterlines(svdvals(K)[1:2:end]; axis = (; xscale = log10, yscale = log10, limits = (nothing, (1e-3, 10))))

# ╔═╡ df598a2e-36dc-4408-9904-25df92ada331
begin
	fig = scatterlines(svdvals(K)[1:2:end] ./ step(domain))
	scatterlines!(svdvals(K)[1:2:end] ./ step(domain), 1:80)
	fig
end
...
manifest_format = "2.0"
project_hash = "8d80694239857bb00fdd4aa5d32c4c3a7de75be9"
