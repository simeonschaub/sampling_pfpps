...
# ╔═╡ f1b4d654-fba6-404f-bd1f-327f24ca6b37
using CairoMakie
...
# ╔═╡ a53a764a-84c6-428a-9d32-8a92c64dcbf6
using YoungTableaux
...
# ╔═╡ f70abfcf-bd31-4486-863b-108f3c97238e
cutoff = 50

# ╔═╡ adaf57e6-213a-45e6-ba67-7e1665d6c90b
q = 0.2

# ╔═╡ f519d5b9-0b47-42e3-a8b5-1ae70aeac36b
ε(x, y) = sign(y - x) / 2

# ╔═╡ 280623a9-19f9-4b46-9dff-2eb16247ff09
let q = q
	global w(x) = q ^ (x / 2)
end
...
# ╔═╡ 13438756-9ca2-4c34-a725-ff1e19eff31a
function skew_dot(u, v)
	return sum(Iterators.product(eachindex(domain), eachindex(domain))) do (i, j)
		w(domain[i]) * w(domain[j]) * ε(domain[i], domain[j]) * u[i] * v[j]
	end
end

# ╔═╡ 53156738-f032-4438-b23e-39c39b4bfa28
norm(f) = √sum(x -> w(x) * f[x + 1]^2, domain)
...
# ╔═╡ 7c08677c-ec37-43b3-a888-a6f0c4f31efe
begin
	v₀ = SymplecticFormVec(fill(1 / √sum(w, domain), cutoff + 1), skew_dot)
	itr = ArnoldiIterator(u -> SymplecticFormVec(domain .* u[], u.skewf), v₀, ModifiedSymplecticGramSchmidtIR(0.5, ESR3m))
	fact = initialize(itr)
	for _ in 1:(N - 1)
		expand!(itr, fact)
	end
	global H = [rayleighquotient(fact); zeros(1, N - 1) normres(fact)]
	global W = stack(getindex, basis(fact).basis)
end

# ╔═╡ 9ea0dc81-8b39-4d49-9892-66fbd3be0292
skew_dot.(eachcol(W), permutedims(eachcol(W)))

# ╔═╡ 15ad1a90-511c-4988-8db2-a885f972ddc8
domain .* W, W * H[1:N, :] + residual(fact)[] * rayleighextension(fact)'

# ╔═╡ aa5d0695-e10f-473f-96a1-b4e5862d706a
function eval_p_all(H::AbstractMatrix, x::AbstractVector, m::Int; init = true)
    T = promote_type(eltype(H), eltype(x))
    q = length(x)

    P = Matrix{T}(undef, q, m + 1)
	s = Vector{T}(undef, q)
    P[:, 1] .= T(init)
    ...
    @views for n = 1:m
        ...
        h = H[n + 1, n]
        @assert h != 0 "Breakdown: H[$(n), $(n + 1)] = 0"
...
# ╔═╡ 45c5fd96-2691-442b-ab6a-d258aef3cc71
# ╠═╡ disabled = true
#=╠═╡
FontFamily(
	Dict(
		:regular => joinpath("/usr/share/texlive/texmf-dist/fonts/opentype/public/newcomputermodern/", "NewCMSans10-Regular.otf"),
		:italic => joinpath("/usr/share/texlive/texmf-dist/fonts/opentype/public/newcomputermodern/", "NewCMSans10-Oblique.otf"),
		:bold => joinpath("/usr/share/texlive/texmf-dist/fonts/opentype/public/newcomputermodern/", "NewCMSans10-Bold.otf"),
		:bolditalic => joinpath("/usr/share/texlive/texmf-dist/fonts/opentype/public/newcomputermodern/", "NewCMSans10-BoldOblique.otf"),
		:math => joinpath("/usr/share/texlive/texmf-dist/fonts/opentype/public/newcomputermodern/", "NewCMSans10-Regular.otf")
	),
) |> set_texfont_family!
  ╠═╡ =#
...
let N = 6, cutoff = 10
	...
	linestyle = repeat([:solid, :dash, :dot, :dashdot, :dashdotdot, (:dot, :loose)], fld1(N, 6))[1:N]
	fig = Figure(; size = (500, 500))

	ax = Axis(fig[1, 1]; limits = ((-0.5, cutoff + 0.5), (-50, 50)))
	hidexdecorations!(ax; grid = false)
	x = range(-0.5, cutoff + 0.5; length = 256)
	series!(ax, broadcast.(Point2f, Ref(x), eachcol(eval_p_all(H, x, N - 1; init = 1 * W[1, 1]))); color)
	scatter!(ax, vec(Point2f.(domain, W[:, 1:N])); color = repeat(color; inner = length(domain)))
	axislegend(ax, [
		[LineElement(; color), MarkerElement(; color, marker = :circle)]
		for (N, color) in zip(0:(N - 1), color)
	], map(N -> L"p_%$N(x)", 0:(N - 1)))

	ax2 = Axis(fig[2, 1]; limits = ((-0.5, cutoff + 0.5), (-.1, 1.2)), xlabel = L"x")
	scatterlines!(ax2, domain, map(w, domain); color = :gray, label = L"w(x)")
	axislegend(ax2; merge = true)
	rowsize!(fig.layout, 2, Relative(0.2))

	Label(fig[0, 1], L"$\beta = 1$\textbf{ Skew-Orthogonal Polynomials w.r.t. }$w(x) = %$(1 - q)^{x / 2}$"; tellwidth = false, font = ax.titlefont, fontsize = 18)

	save("symmetric_growth_sop.pdf", fig)
...
begin
	...
	f = Matrix{Float64}(undef, 2cutoff + 2, 2cutoff + 2)

	ϕ(x, k) = w(x) * W[x + 1, k + 1]
	ψ(x, k) = sum(domain) do y
		ϕ(y, k) * ε(y, x)
	end

    S₁(x, y) = sum(0:(N ÷ 2 - 1)) do k
        ψ(x, 2k) * ϕ(y, 2k + 1) - ψ(x, 2k + 1) * ϕ(y, 2k)
    end
    D₁(x, y) = sum(0:(N ÷ 2 - 1)) do k
        ϕ(x, 2k) * ϕ(y, 2k + 1) - ϕ(x, 2k + 1) * ϕ(y, 2k)
    end
	I₁(x, y) = sum(domain) do z
		S₁(x, z) * ε(y, z)
	end - ε(y, x)
	...
	K = JMatrix(2cutoff + 2) * f
end

# ╔═╡ cee67525-490b-4a3b-80ec-252d0e7da1b4
# ╠═╡ disabled = true
#=╠═╡
begin
	# see https://www.brandeis.edu/mathematics/people/adlerpublications/22.skeworthogpol+randommat.pdf Eq (2.8)
	...
	r = 1/2 * sum(domain) do x
		w(x) * W[x + 1, N]
	end
	function q̂(x, k)
		res = W[x + 1, k + 1]
		if k < N - 1
			res -= W[x + 1, N] / 2r * sum(domain) do x′
				w(x′) * W[x′ + 1, k + 1]
			end
		end
		return res
	end
	Φ(x, k) = sum(domain) do y
		w(y) * ε(y, x) * q̂(x, k)
	end

    S₁(x, y) = w(y) * sum(0:((N - 1) ÷ 2 - 1)) do k
        Φ(x, 2k) * q̂(y, 2k + 1) - Φ(x, 2k + 1) * q̂(y, 2k)
    end + q̂(y, N - 1) / 2r
    D₁(x, y) = w(x) * w(y) * sum(0:((N - 1) ÷ 2 - 1)) do k
        q̂(x, 2k) * q̂(y, 2k + 1) - q̂(x, 2k + 1) * q̂(y, 2k)
    end
	I₁(x, y) = sum(domain) do z
		S₁(x, z) * ε(y, z)
	end - ε(y, x) + Φ(x, N - 1) / 2r
...
# ╔═╡ c9ec425f-0f72-4a2d-9daf-5968fe6a5a18
Partition(reverse(h .- (1:N)))

# ╔═╡ a0e3e0f1-817b-4d7f-8780-bf05eeed0969
begin
	hists1 = [Hist1D(; counttype = Int, binedges = -0.5:40.5) for _ in 1:N, _ in 1:50]
...
# ╔═╡ b656b50f-101c-4381-9298-510a4761ff86
function symmetric_geometric!(W, q)
	for i in axes(W, 1)
		W[i, i] = rand(Geometric(1 - √q))
		@views rand!(Geometric(1 - q), W[(i + 1):end, i])
		@views W[i, (i + 1):end] .= W[(i + 1):end, i]
	end
	return W
end

# ╔═╡ fe2f0c05-201a-4aff-a0d9-63735a64c6fe
function accumulate_corner_growth!(T, W; shifted=true)
...
begin
	...
	@tasks for _ in 1:10000
		@local (W, T) = (Matrix{Int}(undef, N, N), Matrix{Int}(undef, N, N))
		for i in 1:50
			T[N, N] = typemax(Int)
			while T[N, N] > cutoff - N + 1
				symmetric_geometric!(W, q)
				accumulate_corner_growth!(T, W; shifted = false)
			end
			atomic_push!(hists2[1, i], T[N, N])
		end
	end
...
let xlims = (0, 30)
	...
	ax = Axis(fig[1, 1]; limits = (xlims, (0, 1.1 * maximum(bincounts(normalize(hists2_mean[1]))))))
	...
	x = 0:cutoff
	y = map(x) do k
		pfaffian(skewhermitian!(JMatrix(2(cutoff - k) + 2) - K[(2k:(2cutoff + 1)) .+ 1, (2k:(2cutoff + 1)) .+ 1]))
	end
	stairs!(ax, (1:(cutoff + 5)) .- N .+ 0.5, diff([y; ones(5)]); color = :yellow, linewidth = 2, linestyle = :dot)
	...
	], ["PfPP", "Symmetric Corner Growth", "Fredholm Pf"])
	Label(fig[0, 1], L"\textbf{Distribution of }$F(%$N)$\textbf{ sampled from PfPP and directly}"; tellwidth = false, fontsize = 18)
	
	save("symmetric_growth_pfpp.pdf", fig)

	fig
end

# ╔═╡ 4ed66536-4ef3-40fc-845b-dc6d5df23d14
import Images, ColorSchemes, Colors

# ╔═╡ 1622ef2e-87d5-45d0-8f09-df13c51f98dc
let N = 1200
	W, T = Matrix{Int}(undef, N, N), Matrix{Int}(undef, N, N)
	symmetric_geometric!(W, q)
	accumulate_corner_growth!(T, W; shifted = false)

	t_max = 300
	img = map(T) do t
		Colors.ARGB32(t > t_max ? colorant"transparent" : get(ColorSchemes.inferno, (t / t_max)^2))
	end
	save("symmetric_corner_growth.png", img)
	img
end

# ╔═╡ 5ef00dfb-00ae-4f6e-b249-b81b6d3e3420
Y′ = let (λ, Q) = eigen(f; sortby = abs)
	Q[:, isapprox.(λ, 1; rtol = 1e-3)]
end

# ╔═╡ 8a4fd827-f9fd-4d8d-86b5-0ab0b3c9b11c
let (λ, Q) = eigen(f; sortby = abs)
	global Z′ = inv(Q)[isapprox.(λ, 1; rtol = 1e-3), :]
	Q * Diagonal(λ) / Q, f, Y′ * Z′
end

# ╔═╡ deb26363-9342-4d89-8c36-bf93d764ef52
...
# ╔═╡ 528a1645-44ab-4868-b607-31004b0b2046
let (_Q, R) = sr(Y′)
	Q = _Q[:, 1:2N]
	Q' * JMatrix(102) * Q, (R * Z′) * JMatrix(102) * (R * Z′)'
end

# ╔═╡ 1a99e7e8-23b0-4333-a274-95996267cda3
...
# ╔═╡ 64625f1f-863d-41b5-a646-0aca2060a44a
svdvals(f)

# ╔═╡ 825c1091-89e9-4245-83f2-5695801f48d9
eigvals(f, sortby = x -> -abs(x))

# ╔═╡ 36ace2e3-b419-428e-ade3-f3f47be9196d
rank(K)

# ╔═╡ 92770790-d518-404d-9363-464c71a1cc8c
svdvals(Y′)

# ╔═╡ c93aef9c-e528-4692-a6d2-26c563fff24b
let (_U, _S, _V) = svd(f)
	U, S, V = _U[:, 1:2N], _S[1:2N], _V[:, 1:2N]
	@show tr(U * Diagonal(S) * V')
	Q1 = qr(U[3:4, :]').Q[:, 3:end]
	Q1 = qr(U[3:4, :]').Q[:, 3:end]
end

# ╔═╡ f363f463-1897-4ca8-a9c6-2804173a3531
randPfPPproj(Y′, Z′)

# ╔═╡ 07dbc73a-2125-41fe-af39-3be7cd86ebc6
Z′ * Y′

# ╔═╡ 9fa222cd-9cbf-4d2f-8a8c-3a8aa3d1e239
begin
	hists5 = [Hist1D(; counttype = Int, binedges = -0.5:(cutoff + 0.5)) for _ in 1:N, _ in 1:50]
	let
		Y, Z = Y′, Z′
		@tasks for _ in 1:10000
			for i in 1:50
	...
	hists5_mean = map(1:N) do i
		...
		m = mean(c; dims = 2)
		Hist1D(; binedges = -0.5:(cutoff + 0.5), bincounts = vec(m))
	end
...
# ╔═╡ 384bf9ae-ac78-40c9-bb8c-187ee1c1b0b9
let xlims = (0, 30), (hists1_mean, hists1_errors) = (hists5_mean, hists5_errors)
	fig = Figure(; size = (500, 330))
	ax = Axis(fig[1, 1]; limits = (xlims, (0, 1.1 * maximum(bincounts(normalize(hists2_mean[1]))))))
	stairs!(ax, normalize(hists1_mean[1]))
	errorbars!(ax, hists1_errors[1] .- Vec3f(.15, 0, 0); color = Cycled(1), linewidth = 2)
	...
	Label(fig[0, 1], L"\textbf{Distribution of }$F(%$N)$\textbf{ sampled from PfPP and directly}"; tellwidth = false)

	fig
end

# ╔═╡ 3cabe3be-a5b1-4344-b4ff-8a9330b22dba
begin
...
# ╔═╡ 93aacd83-2348-44d5-a22e-35b8045081ea
let
	fig = Figure(; size = (650, 650))
	ax = Axis(fig[1, 1]; yscale = _log10, limits = ((0, 40), (1e-6, 5)))
	tightlimits!(ax)
	for i in 1:N
...
# ╔═╡ 2b372340-2755-4942-a076-8d05cb330fed
heatmap(real.(Y′)'; figure = (; size = (200, 1000)), axis = (; yreversed = true))

# ╔═╡ aef3c185-9648-40f9-ab55-f453be4ba043
lines(0..30, x -> 1 / √(30 - x^2))

# ╔═╡ 528c114b-edc0-4815-8936-e4eb2ba5a5bc
heatmap(f)

# ╔═╡ 7fca76ed-cd35-444e-8824-55a1e67556ff
heatmap(f^2)

# ╔═╡ ada5620c-7bc0-45d6-83a8-17f7b411a4b2
svdvals([
	f[1:2:end, 1:2:end] f[1:2:end, 2:2:end] .- ε.(domain, domain')
	f[2:2:end, :]
])

# ╔═╡ 3b8037e9-ebf1-4349-b1f5-5c277538b6b2
svdvals([
	K[1:2:end, 1:2:end] K[1:2:end, 2:2:end]
	K[2:2:end, 1:2:end] K[2:2:end, 2:2:end] .+ ε.(domain, domain')
])
...
manifest_format = "2.0"
project_hash = "f90f69b98db989183d5759db86a49a21928bbcae"
...
git-tree-sha1 = "1d57589902579bd5588a2bcd205f371e6d501e31"
