...
# ╔═╡ 08426cd8-26ae-11f1-ba30-75e2e9148df4
using CairoMakie, JLD2

# ╔═╡ 8bf35e61-e3e2-4ad7-9a8c-79e9be69b482
using FHist

# ╔═╡ ee62f132-0946-4e41-a569-50582719e8ed
using Statistics

# ╔═╡ 54413816-79e7-485a-9152-870b29fca84c
x = 5:-.05:-8

# ╔═╡ 5ec098ac-23b3-44c2-b0d5-57549729a7f9
samples_goe = load("data/TW1_2nd", "s")

# ╔═╡ 3a04c29b-2dbe-40e4-9129-047adfa6ea57
pdf_goe = load("data/TW1lambda2", "absttt1")

# ╔═╡ 1b247fbb-03fd-42d4-9e52-471a59b118a3
samples_gse = load("data/TW4_2nd", "s4")

# ╔═╡ b8e2400d-0790-4cef-8272-8485a5f8beef
extrema(samples_gse)

# ╔═╡ e0302847-d32b-4f31-8317-87f88a34b0fb
pdf_gse = load("data/TW4lambda2", "ttt")

# ╔═╡ ba461bae-8eb6-465e-bafd-663d9d2a3a2e
function plot_line_ensemble(x, samples, rebin_factor = 1; title = "", xlims)
	binedges = (first(x) - step(x) / 2):step(x):(last(x) + step(x) / 2)
	if step(x) < 0
		binedges = reverse(binedges)
	end
	hists = [Hist1D(x[filter(!=(0), i)]; binedges) for i in eachcol(reshape(samples, 10^4, :))]
	hists1 = rebin.(hists, rebin_factor)
	hists1_mean = let
		c = stack(bincounts.(hists1))
		m = mean(c; dims = 2)
		Hist1D(; binedges = FHist.binedges(hists1[1]), bincounts = vec(m))
	end
	hists1_errors = let
		c = stack(bincounts.(normalize.(hists1)))
		m = mean(c; dims = 2)
		s = std(c; dims = 2)
		Vec3f.(bincenters(hists1[1]), vec(m), vec(s))
	end
	
	fig = Figure(; size = (500, 330))
	ax = Axis(fig[1, 1]; limits = (xlims, (0, 1.1 * maximum(bincounts(normalize(hists1_mean))))))#, title)
	stairs!(ax, normalize(hists1_mean))
	errorbars!(ax, hists1_errors; color = Cycled(1), linewidth = 2)
	Label(fig[0, 1], title; font = ax.titlefont, tellwidth = false, fontsize = 18)

	return fig, ax
end

# ╔═╡ 494ca1cf-8401-4c52-8bb8-5bdf35235cc6
let
	fig, ax = plot_line_ensemble(5:-.05:-8, samples_goe, 3; title = rich("Second largest eigenvalue of the Airy", subscript("1"), " point process"), xlims = (-7, 1))
	lines!(ax, -8:0.05:2, pdf_goe; color = :red, linewidth = 2)
	axislegend(ax, [
		[LineElement(; color = Cycled(1)), LineElement(; color = Cycled(1), points = Point2f[(0.5, 0.2), (0.5, .8)])],
		LineElement(; color = :red, linewidth = 2),
	], ["PfPP", L"f_{\lambda_1}^{(4)}(s)"])
	save("airy1_hist.pdf", fig)
	fig
end

# ╔═╡ 4ba84478-f9fa-4dff-813c-8d12561f0f14
let
	fig, ax = plot_line_ensemble(1:-0.05:-10.05, samples_gse, 2; title = rich("Second largest eigenvalue of the Airy", subscript("4"), " point process"), xlims = (-7, -2))
	lines!(ax, -8:0.05:-1, pdf_gse; color = :red, linewidth = 2)
	axislegend(ax, [
		[LineElement(; color = Cycled(1)), LineElement(; color = Cycled(1), points = Point2f[(0.5, 0.2), (0.5, .8)])],
		LineElement(; color = :red, linewidth = 2),
	], ["PfPP", L"f_{\lambda_2}^{(4)}(s)"])
	save("airy4_hist.pdf", fig)
	fig
end
